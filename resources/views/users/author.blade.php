        ...
        <div class="main_content">
            <div id="main_homepage">
                <div class="homepage_tags">
                    <h1>
                        <p class="text_list_update"><i class="fa fa-font-awesome" aria-hidden="true"></i> {{$author->name}}</p>
                    </h1>
                    <div class="clear"></div>
                </div>
                    ...
                    <table>
                        <tbody>
                            <tr>
                                ...
                                <td>
                                    <ul class="choose">
                                        <li><a class="{{ request()->has('status') && request()->input('status') == 1 ? 'active' : '' }}" href="{{ route('showAuthor', ['slug' => $author->slug]) . '?' . http_build_query(array_merge(request()->query(), ['status' => 1])) }}">{{__('string.ongoing')}}</a></li>
                                        <li><a class="{{ request()->has('status') && request()->input('status') == 2 ? 'active' : '' }}" href="{{ route('showAuthor', ['slug' => $author->slug]) . '?' . http_build_query(array_merge(request()->query(), ['status' => 2])) }}">{{__('string.completed')}}</a></li>
                                    </ul>
                                </td>
                            </tr>
                        </tbody>
